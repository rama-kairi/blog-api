...
async def get_current_user(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    is_access_token_valid = db.query(models.auth.Session).filter_by(access_token=token).first()

    if not is_access_token_valid:
        raise HTTPException(status_code=401, detail="Token invalid.")

    # Get the User from Token
    user = crud.auth.auth.get_by_any(db, email=crud.auth.auth.decode_jwt_token(token))
    if not user:
...
def base64_to_image(data):
    ...
    if extension not in extension_list:
        raise HTTPException(
            status.HTTP_400_BAD_REQUEST, "Image should be in jpeg or png format only."
        )
...
def upload_to_cloudinary(base64_image: str):
    ...
    cloudinary_response = httpx.post(
        "https://api.cloudinary.com/v1_1/simply-jet-sa/image/upload",
