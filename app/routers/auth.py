...
from app.schemas.auth import Login, Token, TokensResponse, UserDb, UserIn, UserOut
...
def signup(
    ...
    # Create User dict
    db_user = User(**UserDb(**user_obj.dict()).dict(), password=hashed_password)
...
def login(
    ...
    *,
    ...
    if "user" not in user_groups:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only User Can Login in this Panel."
        )
    # Authenticate User with email and password
    isAuthenticated = crud.auth.auth.authenticate(db, email=user.username, password=user.password)
    ...
    # Update Last Login
    crud.auth.auth.update(db=db, db_obj=user_instance, obj_in={"last_login": func.now()})

    # Create Sessions
    # session_data = deps.create_session(
    #     access_token, refresh_token, user.email, request, db)

    bt.add_task(deps.create_session, access_token, refresh_token, user.username, request, db)
...
def logout(
    token: Token, db: Session = Depends(get_db), current_user: UserOut = Depends(get_current_user)
):
...
def refresh(refresh_token: str, db: Session = Depends(get_db)):
    ...
    is_refresh_token_valid = (
        db.query(models.auth.Session).filter_by(refresh_token=refresh_token).first()
    )
    if not is_refresh_token_valid:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token Invalid.")

    # Generate Access & Refresh Tokens
    access_token, refresh_token, _ = crud.auth.auth.refresh_token(refresh_token)
